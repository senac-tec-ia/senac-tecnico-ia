...
4. As configurações abaixo já estão definidas em `vercel.json` — **não altere nada no painel**:

| Campo | Valor |
|---|---|
| Framework Preset | **Other** (detectado automaticamente via `vercel.json`) |
| Build Command | `npm run build` |
| Output Directory | `dist` |
| Install Command | `npm install` |
| Node.js Version | 20 (definido em `.nvmrc`) |

5. Clique em **Deploy**

> Para atualizar a apresentação, basta fazer push na branch `master` — a Vercel faz redeploy automaticamente.

### Testando o build localmente antes do deploy
...
Acesse `http://localhost:3000` para verificar o resultado.
